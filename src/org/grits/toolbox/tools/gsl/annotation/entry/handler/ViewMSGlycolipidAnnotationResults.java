...
/**
 * Create a new MS dialog for DANGO annotation (MSGlycolipidAnnotation).
 * Following org.grits.toolbox.entry.ms.annotation.glycan.handler.ViewMSGlycanAnnotationResults
 * @see org.grits.toolbox.entry.ms.annotation.glycan.handler.ViewMSGlycanAnnotationResults
...
//	public final static String VIEW_ID = "plugin.ms.annotation.glycan.views.MSGlycanAnnotationMultiPageViewer";
	...
	public boolean canExecute(@Named(IServiceConstants.ACTIVE_SELECTION) Object object, @Named(IServiceConstants.ACTIVE_PART) MPart part) {
		...
		if (entry != null) {
			if ( entry.getProperty().getType().equals( MSGlycolipidAnnotationProperty.TYPE )  ||
		...
		if (part != null && part.getObject() instanceof MSGlycolipidAnnotationMultiPageViewer)
			...
			for (MPart mPart: partService.getParts()) {
				if (mPart.getObject() instanceof MSGlycolipidAnnotationMultiPageViewer) {
